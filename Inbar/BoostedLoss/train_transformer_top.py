...
def train_step(model, dataloader, cost, optimizer, epoch, device, penalty=None, boost_=boost, apply_penalty=False):
    ...
    return distributed_loss, distributed_bce, distributed_mse
...
def train_model(model, train_loader, test_loader, loss, optimizer, train_sampler, num_epochs=100, device='cpu',patience=5, penalty=None, output_dir="", boost_=boost, apply_penalty=False, save_tag=""):
    ...
    for epoch in range(num_epochs):
        ...
        if device == 0:
            print(f'Epoch [{epoch+1}/{num_epochs}] Loss: {losses["train_loss"][-1]:.4f}, Val Loss: {losses["val_loss"][-1]:.4f},  BCE: {losses["train_BCE"][-1]:.4f}, Val BCE: {losses["val_BCE"][-1]:.4f} MSE: {losses["train_MSE"][-1]:.4f}, Val MSE: {losses["val_MSE"][-1]:.4f}')
        ...
        debug = False 
        if debug:
            print(f"Rank: {device}, Train Loss: {losses['train_loss'][-1]:.5f}, Validation Loss: {losses['val_loss'][-1]:.5f}")
            print(f"Rank: {device}, Best Loss: {tracker['bestValLoss']}, Best Epoch: {tracker['bestEpoch']}")
...
def main(rank, world_size, args=None):
    ...
    # distributed loader
    sampler_train = DistributedSampler(train_data, shuffle=True, num_replicas=world_size, rank=rank)
    sampler_test = DistributedSampler(test_data, shuffle=False, num_replicas=world_size, rank=rank)
